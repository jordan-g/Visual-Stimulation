...
class StimDialog():
    ...
    def ShowDialog(self, controller, stim_index):
        ...
        if stim_index != None:
            ...
            self.stim_parameters = {'init_x_pos': 0.0,
                                    'init_y_pos': 0.0,
                                    'l_v': 150,
                                    'contrast': 1.0}
    ...
    def populate_stim_choice_panel(self):
        ...
        self.stim_chooser.Items.AddRange(("Looming Dot", "Moving Dot", "Grating", "Delay", "Black Flash", "White Flash"))
    ...
    def populate_stim_param_panel(self):
        ...
        if self.stim_type == "Looming Dot":
            self.add_stim_param_to_window('init_x_pos', 'Initial x position (deg)')
            self.add_stim_param_to_window('init_y_pos', 'Initial y position (deg)')
    ...
    def are_valid_params(self, stim_type, stim_params):
        ...
        if stim_type == "Looming Dot":
            stim_params_are_valid = (is_number(stim_params['init_x_pos'])
                                     and is_number(stim_params['init_y_pos'])
            ...
            stim_params_are_valid = (is_nonnegative_number(stim_params['radius'])
                                     and is_number(stim_params['init_x_pos'])
                                     and is_number(stim_params['init_y_pos'])
...
class ParamWindow(Form):
    ...
    def __init__(self, controller):
        ...
        self.Icon = Icon('param_window_icon.ico')
